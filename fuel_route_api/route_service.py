...
from .schema import CoordinateSchema, GeocodeInputSchema, GeocodeOutputSchema, RouteRequestSchema
from .env import TOMTOM_BASE_URL, TOMTOM_API_KEY, MAPBOX_BASE_URL, MAPBOX_API_KEY, GEOAPIFY_API_KEY, GEOAPIFY_BASE_URL
    ...
    async def get_mapbox_route(self, data: RouteRequestSchema) -> Dict:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params) as response:
                ...
                route_points = [{'latitude': lat, 'longitude': lon}
                ...
                await self.cache_deps.set_from_cache(cache_key, normalized_data, timeout=3600)
                return normalized_data

    async def geocode_address(self, address: str, city: str, state: str) -> tuple[float, float]:
...
class TomTomService:
    @inject
    def __init__(self, cache_deps: CacheDependencies, deps: CRUDDependencies, cache_key_deps: CacheKeyDependencies):
        self.cache_deps = cache_deps
        self.cache_key_deps = cache_key_deps
        self.deps = deps
    ...
    async def get_tomtom_route(self, data: CoordinateSchema) -> Dict:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params) as response:
                ...
                if response.status != 200:
                    print(
                        f"❌ TomTom API Error ({response.status}): {route_data}")
                    raise HttpError(

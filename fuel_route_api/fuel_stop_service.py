...
class FuelStopService:
    ...
    async def find_current_optimal_fuel_stops(self,route_coords, vehicle_range_miles=500):
        cache_key = await self.cache_key_deps.generate_cache_key(
            {"type": "current", "route_coords": route_coords, "range": vehicle_range_miles}
        )

        cached = await self.cache_deps.get_from_cache(cache_key)
        if cached:
            logger.info(f" Cache hit for current optimal stops")
            return cached
        normalized_coords = [(c["lon"], c["lat"]) if isinstance(
            c, dict) else tuple(c) for c in route_coords]
        ...
        try:
            stations = await sync_to_async(list)(
                FuelStation.objects.filter(
                    location__distance_lte=(route_line,miles_to_degrees)) .only("truckstop_name", "city", "retail_price", "location").order_by("retail_price")[:10]
            )
    ...
    async def find_optimal_fuel_stops(self, route_points: List[Dict]) -> List[Dict]:
        ...
        for i, point in enumerate(sampled_points):
            ...
            if total_distance >= 500 or i == len(sampled_points) - 1:
                location = p2
                logger.info(f" Querying fuel stations near({location.y}, {location.x}), total_distance={total_distance:.2f} mi")

                try:
                    stations = await sync_to_async(list)(
                        FuelStation.objects
                        .filter(location__distance_lte=(location, D(mi=50))).only("opis_truckstop_id", "truckstop_name", "retail_price", "location").annotate(dist=Distance("location", location)).order_by("retail_price", "dist")[:1]
                    )
                except DatabaseError as e:
                ...
                if stations:
                    ...
                    if cheapest.id not in seen_station_ids:
                        ...
                        last_stop_index = route_points.index (point) if point in route_points else last_stop_index
                        total_distance = 0
                        if len(fuel_stops) >= 3:  
                            break
                else:
                    logger.warning(f"No fuel stations found near ({location.y}, {location.x})")
        ...
        logger.info(f"Fuel stop search complete. Found {len (fuel_stops)} stops: {fuel_stops}")
        return fuel_stops
    ...
    async def calculate_fuel_costs(self, total_distance_miles: float, fuel_stops: List[Dict]) -> Dict:
        logger.info(f"Calculating fuel cost for {total_distance_miles:.2f} miles using {len(fuel_stops)} stops.")

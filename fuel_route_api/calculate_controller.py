...
class CalculateRouteController:
    ...
    async def fetch_route_data(self, coord: RouteRequest):
        ...
        route_data = await self.geoapify_service.get_geoapify_route(coord, mapbox_format=False)
        ...
        fuel_stops = await self.fuel_service.find_current_optimal_fuel_stops(route_points)
        cost_summary = await self.fuel_service.calculate_fuel_costs(distance_miles, fuel_stops)

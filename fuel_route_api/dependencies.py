...
class ExistingDependencies:
    async def async_check_existing(
        self, model, raise_error_if_exists=True, error_field="Record", **kwargs
    ):
        obj = await model.objects.filter(**kwargs).aexists()
        if raise_error_if_exists and obj:
            raise HttpError(status_code=409, message=f"{error_field} already exists.")
        if not raise_error_if_exists and not obj:
            raise HttpError(status_code=404, message=f"{error_field} not found.")
        return obj
